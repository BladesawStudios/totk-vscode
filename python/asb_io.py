...
def _compress_bytes(
    data: bytes, internal_path: str, romfs_path: str, was_compressed: bool
) -> bytes:
    ...
    with _quiet_asb():
        ...
        if baev_internal:
            try:
                ...
                print(
                    f"Warning: Failed to import BAEV from {baev_internal}: {e}\n{traceback.format_exc()}",
                    file=sys.stderr,
                )
...
def write_asb_bytes(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if baev_internal and baev_path.is_file():
            ...
            new_baev_bytes = _compress_bytes(
                new_baev_bytes, baev_internal, romfs_path, was_baev_zstd
            )
    ...
    with _quiet_asb():
        ...
        if baev_path and os.path.isfile(baev_path):
            try:
                ...
                print(
                    f"Warning: Failed to import BAEV from {baev_path}: {e}\n{traceback.format_exc()}",
                    file=sys.stderr,
                )
...
def write_asb_disk(file_path: str, editor_text: str, romfs_path: str = "") -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if baev_path and out_baev.is_file():
            ...
            was_baev_zstd = orig_baev.startswith(b"\x28\xb5\x2f\xfd")
            new_baev_bytes = _compress_bytes(
                out_baev.read_bytes(), baev_path, romfs_path, was_baev_zstd
            )
