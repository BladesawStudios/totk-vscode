...
def _get_addr_block_linear(
    x: int, y: int, image_width_in_bytes: int, bytes_per_pixel: int, block_height: int
) -> int:
...
def _deswizzle_block_linear(
    width: int, height: int, blk_w: int, blk_h: int, bpp: int, block_height_log2: int, data: bytes
) -> bytes:
...
def _deswizzle_pitch_linear(
    width: int, height: int, blk_w: int, blk_h: int, bpp: int, data: bytes
) -> bytes:
    width_in_blocks = _div_round_up(width, blk_w)
    height_in_blocks = _div_round_up(height, blk_h)
...
def _decode_pixels(
    linear_data: bytes, width: int, height: int, decoder_key: str, blk_w: int, blk_h: int
...
def render_texture_to_png(bntx_data: bytes, texture_name: str) -> str | None:
    ...
    _log(
        f"Rendering {tex.name} ({tex.width}x{tex.height}, {fmt_name}, "
